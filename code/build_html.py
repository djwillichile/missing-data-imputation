...
.sb{{position:fixed;top:0;left:0;width:var(--sw);height:100vh;overflow-y:auto;background:var(--bg2);border-right:1px solid var(--bd);padding:2rem 1.25rem;z-index:100}}
...
th{{font-weight:600;text-align:left;padding:.75rem 1rem;border-bottom:2px solid var(--bd)}}
td{{padding:.65rem 1rem;border-bottom:1px solid var(--bl);vertical-align:top}}
tr:hover td{{background:var(--bg2)}}
pre{{background:var(--bgc);border:1px solid var(--bl);border-radius:8px;padding:1.25rem 1.5rem;overflow-x:auto;margin:1.5rem 0;font-size:.85rem;line-height:1.6}}
pre code{{font-family:var(--fc);background:none;padding:0;border:none}}
code{{font-family:var(--fc);font-size:.88em;background:var(--bgc);padding:.15em .4em;border-radius:4px;border:1px solid var(--bl)}}
...
.btt{{position:fixed;bottom:2rem;right:2rem;width:40px;height:40px;background:var(--pr);color:#fff;border:none;border-radius:50%;cursor:pointer;font-size:1.1rem;display:none;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,.15);z-index:50}}
.btt.v{{display:flex}}
@media(max-width:900px){{.sb{{display:none}}.mn{{margin-left:0;padding:2rem 1.25rem}}}}
</style>
</head>
<body>
<aside class="sb">
